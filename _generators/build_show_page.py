...
# ============================================================
SHOW = dict(
    slug='novy-spektakl',                       # -> /novy-spektakl/
    title='Название спектакля',
    genre='Драматический спектакль',             # строка над заголовком (eyebrow)
    age='12+',                                    # возрастная маркировка (436-ФЗ)
    tagline='Одна фраза-теглайн о спектакле — то, что цепляет взгляд в hero.',
    description='Более развёрнутое описание для мета-тега, Open Graph и секции «О проекте» (1–3 предложения).',
    director=dict(
        role='Режиссёр',
        name='Имя Фамилия',
        slug=None,                                # 'bychkova' если есть страница /bychkova, иначе None
    ),
    city_date='Дата уточняется · Санкт-Петербург',  # для параллели с декой; на
                                                       # сайте своего слота пока нет —
                                                       # даты идут через секцию
                                                       # «Ближайшие показы» ниже
    hero_image='https://res.cloudinary.com/kostyamoshnikov/image/upload/w_1600,q_auto,f_auto/ЗАМЕНИТЬ.jpg',
    team=[                                        # можно оставить пустым списком
        dict(role='Режиссёр', name='Имя Фамилия'),
    ],
    gallery=[],                                    # список URL фото (без w_600/w_1200 — подставится)
)
# ============================================================
