...
SHOW = dict(
    ...
    # ⚠️ ПРЕДУПРЕЖДЕНИЕ ОБ ОСОБЕННОСТЯХ ПОКАЗА (pack-v362).
    # Раздел 5 Правил посещения обязывает Компанию приводить существенные
    # особенности на странице мероприятия — стробоскоп, громкий звук,
    # сценический дым и запахи, открытый огонь, вовлечение зрителя и
    # физический контакт с исполнителем. Возрастная маркировка этого НЕ
    # заменяет: 16+ ничего не говорит человеку со светочувствительной
    # эпилепсией. Пустая строка — блок не выводится; заполнять по факту
    # технического райдера показа, а не «на всякий случай»: предупреждение
    # обо всём подряд читают так же, как не читают ничего.
    warning='',
...
def warning_block(s):
    """Блок предупреждения об особенностях показа. Пусто -> ничего."""
    w = (s.get('warning') or '').strip()
    if not w:
        return ''
    return ('<p class="show-warning" role="note">'
            '<strong>Обратите внимание:</strong> ' + esc(w) + ' '
            '<a href="/visiting-rules#osobennosti">Подробнее в Правилах посещения</a>.'
            '</p>')
...
.btn-outline{{font-family:'Montserrat',sans-serif;font-size:12px;letter-spacing:.22em;text-transform:uppercase;border:1px solid rgba(214,181,122,.5);color:var(--bone);padding:15px 30px;text-decoration:none;display:inline-block;transition:all .3s;background:none;-webkit-appearance:none;appearance:none;cursor:pointer}}
</style>
